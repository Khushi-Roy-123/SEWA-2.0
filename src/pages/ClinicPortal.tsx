...
import { Search, QrCode, User, Shield, AlertCircle, Loader2, Plus, Video, VideoOff } from 'lucide-react';
import FaceCapture from '../components/FaceCapture';
import { findMatchingUser } from '../lib/faceRecognition';
import QRScanner from '../components/QRScanner';
import LiveFaceQueue from '../components/LiveFaceQueue';
    ...
    return (
        <div className="flex flex-col h-screen bg-slate-100">
            ...
            <div className="bg-white px-6 py-4 border-b border-slate-200 flex flex-col sm:flex-row sm:items-center justify-between gap-4 z-10 shrink-0">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className={`flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-sm transition-all shadow-md active:scale-95 ${isCameraMode
                            ? 'bg-red-500 text-white hover:bg-red-600'
                            : 'bg-sky-50 text-sky-700 hover:bg-sky-100 border-2 border-sky-200'
                            }`}
                    >
                        {isCameraMode ? <VideoOff size={18} /> : <Video size={18} />}
                        {isCameraMode ? 'Stop Camera' : 'Live Camera'}
                    </button>
                    <button
            ...
            <div className="flex flex-1 overflow-hidden bg-slate-50">
                ...
                {/* Right: Camera + Patient Console (side by side when camera is on) */}
                <div className={`flex-1 relative overflow-hidden flex ${isCameraMode ? 'flex-row' : 'flex-col'}`}>
                    {/* Live Camera Panel — sits on left side when active */}
                    {isCameraMode && (
                        <div className="w-2/5 border-r border-slate-200 flex flex-col shrink-0">
                            ...
                            <div className="flex-1 bg-slate-900">
                                <LiveFaceQueue clinicId={currentUser!.uid} />
                            </div>
                        </div>
                    )}

                    {/* Patient Console — always visible */}
                            ...
                            <div className="flex-1 overflow-y-auto">
                                ...
                                <div className="p-6 sm:p-8 space-y-8">
                                    ...
                                    <div>
                                        ...
                                        <div className={`grid gap-4 ${isCameraMode ? 'grid-cols-1' : 'grid-cols-1 lg:grid-cols-2'}`}>
